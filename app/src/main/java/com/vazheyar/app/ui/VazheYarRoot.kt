...
private fun ReviewScreen(vm: MainViewModel) {
    ...
    LaunchedEffect(dueCount) {
        val impliedTotal = reviewedInSession + dueCount
        if (impliedTotal > sessionTotal) sessionTotal = impliedTotal
        if (sessionTotal == 0 && dueCount > 0) sessionTotal = dueCount
    }
    ...
    val currentNumber = (reviewedInSession + 1).coerceAtMost(sessionTotal.coerceAtLeast(1))

    fun submitReview(rating: ReviewRating) {
        reviewedInSession += 1
        vm.review(card, rating)
    }
    ...
    Column(
        ...
        Text("$currentNumber of ${sessionTotal.coerceAtLeast(dueCount)}", style = MaterialTheme.typography.labelLarge)
        Text("Tap the card to flip • FSRS-6")

        FlipFlashcard(
            card = card,
            onSpeakWord = { speaker.speakWord(card.word) },
            onSpeakExample = { speaker.speakExample(card.exampleEn) }
        )

        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ReviewButton("Again", previews[ReviewRating.AGAIN].orEmpty(), Modifier.weight(1f)) {
                submitReview(ReviewRating.AGAIN)
            }
            ReviewButton("Hard", previews[ReviewRating.HARD].orEmpty(), Modifier.weight(1f)) {
                submitReview(ReviewRating.HARD)
            }
        }
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
...
private fun FlipFlashcard(
    ...
    Card(
        ...
        AnimatedContent(targetState = flipped, label = "flip-card") { back ->
            if (back) CardBack(card, onSpeakExample) else CardFront(card, onSpeakWord)
        }
    }
}
...
@Composable
private fun CardBack(card: FlashcardEntity, onSpeakExample: () -> Unit) {
    Column(
        Modifier.fillMaxSize().padding(28.dp),
        horizontalAlignment = Alignment.CenterHorizontally,
        verticalArrangement = Arrangement.Center
        ...
        if (card.exampleEn.isNotBlank()) {
            Spacer(Modifier.height(16.dp))
            FilledTonalButton(onClick = onSpeakExample) {
                Icon(Icons.Default.VolumeUp, contentDescription = null)
                Spacer(Modifier.width(8.dp))
...
private fun LibraryScreen(vm: MainViewModel) {
    ...
    LaunchedEffect(cards) {
        val existingIds = cards.mapTo(hashSetOf()) { it.id }
        selectedIds = selectedIds.intersect(existingIds)
        if (selectionMode && selectedIds.isEmpty() && cards.isEmpty()) selectionMode = false
    }
    ...
    if (confirmBulkDelete) {
        AlertDialog(
            ...
            title = { Text("Delete selected flashcards?") },
            text = { Text("Delete ${selectedIds.size} selected flashcard(s) and their review history? This cannot be undone.") },
            confirmButton = {
                Button(
                    onClick = {
                        vm.deleteCards(selectedIds)
    ...
    LazyColumn(
        ...
        item {
            ...
            Row(
                ...
                TextButton(
                    ...
                    Text(if (selectionMode) "Cancel selection" else "Select multiple")
                }
            }
            ...
            if (selectionMode) {
                Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.secondaryContainer)) {
                    Column(Modifier.fillMaxWidth().padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            OutlinedButton(
                                onClick = { selectedIds = selectedIds + visible.map { it.id } },
        ...
        items(visible, key = { it.id }) { card ->
            ...
            Card(
                modifier = Modifier.fillMaxWidth().clickable(enabled = selectionMode) {
...
private fun SettingsScreen(vm: MainViewModel) {
    ...
    val themeMode by vm.themeMode.collectAsStateCompat()
    val failed by vm.failedCount.collectAsStateCompat()
    ...
    Column(
        ...
        Card {
            Column(Modifier.padding(14.dp)) {
                ProviderRadio("System default", "Follow the phone's light/dark setting", themeMode == AppThemeMode.SYSTEM) { vm.setThemeMode(AppThemeMode.SYSTEM) }
                ProviderRadio("Light", "Always use the light theme", themeMode == AppThemeMode.LIGHT) { vm.setThemeMode(AppThemeMode.LIGHT) }
                ProviderRadio("Dark", "Always use the dark theme", themeMode == AppThemeMode.DARK) { vm.setThemeMode(AppThemeMode.DARK) }
            }
        }
        ...
        Text("Database v2 • FSRS-6 • Gemini + Groq • Light/Dark theme", style = MaterialTheme.typography.bodySmall)
        Spacer(Modifier.height(16.dp))
    }
}
