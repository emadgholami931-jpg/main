...
// Release APKs must always use the same permanent signing key.  We deliberately
// refuse to assemble a release when signing configuration is incomplete so an
// unsigned or accidentally re-signed APK can never be published as an update.
...
if (releaseTaskRequested && !hasCompleteReleaseSigning) {
    error(
        "Release signing is not configured. Set ANDROID_KEYSTORE_FILE, " +
            ...
            "Never publish a debug APK as a production update."
    )
}
...
android {
    ...
    defaultConfig {
        // Never change this ID for future releases. Android uses it together
        // with the signing certificate to identify updates of the same app.
