...
- Review uses the full due-count for the session; the 50-card list cap is only a memory/UI queue window and is not shown as the total.
- Android Text-to-Speech reads both the English headword and the English example sentence with `Locale.US`.
- Words supports explicit multi-select and bulk deletion; card rows and their review logs are removed together in one Room transaction.
- `AppThemeSettings` stores only the non-sensitive appearance choice (`SYSTEM`, `LIGHT`, or `DARK`) in private SharedPreferences. Theme changes apply immediately and System mode follows the device setting.
